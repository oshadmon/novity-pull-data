...
def __format_results(results:dict, tags_list:list, monitor_list):
    for tag in tags_list:
        ...
        if 'multiply' in tag['tag']:
            results[tag['tag']['name']] = results[tag['tag']['name']] * tag['tag']['multiply']
...
def main():
    ...
    if args.raw_data is True:
        ...
        if args.file_store:
            __write_data(results=output, plant=args.plant_code, query_type='raw')
        else:
    ...
    if args.aggregate_data is True:
        ...
        group_by = []
        for tag in tag_policies:
            if tag['tag']['data type'] in ['numeric', 'int', 'float', 'double']:
        ...
        if args.file_store:
            __write_data(results=output, plant=args.plant_code, query_type='aggregate')
        else:
            __print_results(results=output)
